...
/* Browser console bridge. The harness (iframe JS) defines globalThis.CPM with
   putch(ch), getch() -> int (-1 if no key queued), kbhit() -> 0/1, clrscr().
   _getch blocks by yielding to the browser via emscripten_sleep (needs
   -sASYNCIFY) until CPM.getch() returns a key. */
...
void _console_init(void) {
    /* No termios in the browser; the harness owns the terminal. */
}
...
int _kbhit(void) {
    // Programs that poll for input (real-time games like Rogue) spin on
    // _kbhit without ever calling _getch, so without a yield here the wasm
    // never returns to the browser and the tab freezes. Yield periodically
    // (cheap: emscripten_sleep(0) just reschedules) so the event loop runs
    // and queued keys/paint happen, without throttling the poll noticeably.
    static unsigned n = 0;
    if ((++n & 0x1FF) == 0) emscripten_sleep(0);
    return cpm_js_kbhit();
}

uint8 _getch(void) {
    int c;
    /* Yield to the browser event loop until a key is queued (ASYNCIFY). */
    while ((c = cpm_js_getch()) < 0) {
        emscripten_sleep(8);
    }
    return (uint8)c;
}
